// theia: for 64 bit; start a process replay from userspace.
#include <getopt.h>
#include <stdio.h>
#include <fcntl.h>
#include <unistd.h>
...
int main(int argc, char *argv[]) 
{
  ...
  if (libdir) {
    ...
    //	strcat (ldpath, "/ld-linux-x86-64.so.2");
  ...
  printf(" theia_replay_register pid %d follow %d\n", pid, follow_splits);

//1. ioctl to notify kernel which process will be replayed.
  rc = theia_replay_register (fd, attach_pin, 
    follow_splits, save_mmap, argv[base], libdir);

  if (rc < 0) {
    perror ("theia_replay_register error");
    return -1;
  }
