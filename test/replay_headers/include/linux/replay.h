...
int fork_replay (char * logdir, const char *const *args,
		const char *const *env, char* linker, int save_mmap, int fd,
		int pipe_fd);
...
long get_used_addresses (struct used_address * plist, int listsize);
...
long get_replay_stats (struct replay_stats * ustats);
...
long get_filemap(int fd, loff_t offset, int size, void * entries, int num_entries);
