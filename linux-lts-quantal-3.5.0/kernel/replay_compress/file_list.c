...
int init_file_list (struct file_list_struct* file_list, char* file_list_path) {
  ...
  THEIA_SWAP_CREDS_TO_ROOT;
	...
	pr_debug ("Pid %d begins init_file_list, filename %s\n", current->pid, file_list_path);
	set_fs (KERNEL_DS);

	fd = sys_open (file_list_path, O_RDONLY, 0);
	if (fd < 0) {
		pr_err("init_file_list , cannot open file %s, return %d\n", file_list_path, fd);
		goto exit;
	}
	file = fget (fd);
	...
	if (copyed != sizeof (int)) {
		pr_err("init_file_list: tried to read count for ignored_file_list, got rc %d\n", copyed);
		rc = copyed;
		goto exit;
	}
	...
	if (count > 0) {
		...
		if (file_list->ignored_list == NULL) {
			pr_err("init_file_list: unable to allocate ignored_list with size %lu\n", count*sizeof(struct file_list_name_struct));
			rc = -ENOMEM;
			goto exit;
		}
		...
		if (copyed != count*sizeof(struct file_list_name_struct)) {
			pr_err("init_file_list: ftried to read ignored_list for ignored_file_list, got rc %d, expected:%lu\n", copyed, count*sizeof(struct file_list_name_struct));
			rc = copyed;
			goto exit;
		}
	} else {
	...
	copyed = vfs_read (file, (char*) &count, sizeof(int), &pos);
	if (copyed != sizeof (int)) {
		pr_err("init_file_list: ftried to read count for modify_file_list, got rc %d\n", copyed);
		rc = copyed;
		goto exit;
	}
	...
	if (count > 0) {
		...
		if (file_list->modify_list == NULL) {
			pr_err("init_file_list: unable to allocate modify_list with size %lu\n", count*sizeof(struct file_list_name_struct));
			rc = -ENOMEM;
			goto exit;
		}
		copyed = vfs_read (file, (char*)file_list->modify_list, count*sizeof(struct file_list_name_struct), &pos);
		if (copyed != count*sizeof(struct file_list_name_struct)) {
			pr_err("init_file_list: ftried to read modify_list for modify_file_list, got rc %d, expected:%lu\n", copyed, count*sizeof(struct file_list_name_struct));
			rc = copyed;
			goto exit;
		}
	} else {
	...
	pr_debug("Pid %d init_file_list done.\n", current->pid);

exit:
	if (fd >= 0) {
		rc = sys_close (fd);
		if (rc < 0) pr_err("init_file_list: close returns %d\n", rc);
	}
	if (file) fput(file);
	set_fs(old_fs);
  THEIA_RESTORE_CREDS;
