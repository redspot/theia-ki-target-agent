...
bool get_ip_port_sockaddr(struct sockaddr __user *sockaddr, int addrlen, char *ip, u_long *port, char *sun_path, sa_family_t *sa_family);

bool get_ip_port_sockfd(int sockfd, char *ip, u_long *port, char *sun_path, sa_family_t *sa_family, bool is_peer);

inline bool get_peer_ip_port_sockfd(int sockfd, char *ip, u_long *port, char *sun_path, sa_family_t *sa_family)
{
  return get_ip_port_sockfd(sockfd, ip, port, sun_path, sa_family, true);
}

inline bool get_local_ip_port_sockfd(int sockfd, char *ip, u_long *port, char *sun_path, sa_family_t *sa_family)
{
  return get_ip_port_sockfd(sockfd, ip, port, sun_path, sa_family, false);
}
...
bool file2uuid(struct file *file, char *uuid_str, int fd)
{
  ...
  if (file)
  {
    ...
    if (S_ISSOCK(mode))
    {
      ...
      {
        sockfd_put(sock);
        if (false == get_peer_ip_port_sockfd(fd, ip, (u_long *)&port, sun_path, &sa_family)) 
          return false;
        if (false == get_local_ip_port_sockfd(fd, local_ip, (u_long *)&local_port, local_sun_path, &sa_family))
          return false;
...
void get_user_callstack(char *buffer, size_t bufsize)
{
  ...
  for (i = 0; i < trace.nr_entries; ++i)
  {
    ...
    if (vma->vm_file)
    {
      ...
      ret = file2uuid(vma->vm_file, uuid_str, -1);
      if(ret) {
...
void packahgv_mount(struct mount_ahgv *sys_args)
{
  ...
  if (theia_logging_toggle)
  {
    ...
    size = snprintf(buf, buf_size, "startahg|%d|%d|%ld|%s|%s|%s|%s|%lu|%ld|%d|%ld|%ld|%u|endahg\n",
...
void theia_mount_ahg(char __user *dev_name, char __user *dir_name, char __user *type, unsigned long flags, long rc)
{
...
long theia_sys_mount(char __user *dev_name, char __user *dir_name, char __user *type, unsigned long flags, void __user *data)
{
  long rc;
...
record_pipe(int __user *fildes)
{
  ...
  if (rc == 0)
  {
    ...
    if (rc >= 0)
    {
      theia_pipe_ahg((u_long)rc, *pretval, *(pretval + 1));
    }
...
void packahgv_sendto(struct sendto_ahgv *sys_args)
{
  ...
  if (theia_logging_toggle)
  {
    ...
    if (sys_args->sock_fd != -1) {
      ...
      if (addr2uuid(NULL, (struct sockaddr*)&sys_args->dest_addr, uuid_str) == false)
        goto err;
    }
...
void packahgv_recvfrom(struct recvfrom_ahgv *sys_args)
{
  ...
  if (theia_logging_toggle)
  {
    ...
    if (sys_args->sock_fd != -1) {
      ...
      if (addr2uuid(NULL, (struct sockaddr*)&sys_args->src_addr, uuid_str) == false)
        goto err;
    }
...
void theia_connect_ahg(long rc, int fd, struct sockaddr __user *uservaddr, int addrlen)
{
  ...
  if (sock) {
    sockfd_put(sock);
    if (false == get_peer_ip_port_sockfd(fd, pahgv_connect->ip, &(pahgv_connect->port), pahgv_connect->sun_path, &(pahgv_connect->sa_family))) {
      pahgv_connect->sock_fd = -1;
      if (false == get_ip_port_sockaddr(uservaddr, addrlen, pahgv_connect->ip, &(pahgv_connect->port), pahgv_connect->sun_path, &(pahgv_connect->sa_family))) {
        KFREE(pahgv_connect);
        return;
      }
    }
...
void theia_accept_ahg(long rc, int fd, struct sockaddr __user *upeer_sockaddr, int __user *upeer_addrlen)
{
  ...
  if (sock) {
    sockfd_put(sock);
    if (false == get_peer_ip_port_sockfd(fd, pahgv_accept->ip, &(pahgv_accept->port), pahgv_accept->sun_path, &(pahgv_accept->sa_family))) {
      if (false == get_ip_port_sockaddr(upeer_sockaddr, *upeer_addrlen, pahgv_accept->ip, &(pahgv_accept->port), pahgv_accept->sun_path, &(pahgv_accept->sa_family))) {
        KFREE(pahgv_accept);
        return;
      }
    }
...
void theia_sendto_ahg(long rc, int fd, void __user *buff, size_t len, unsigned int flags, struct sockaddr __user *addr, int addr_len)
{
  ...
  if (sock && (sock->type == SOCK_STREAM || sock->type == SOCK_SEQPACKET)) {
    ...
    if (addr != NULL && addr_len > 0) {
      ...
      if (copy_from_user((char*)&pahgv_sendto->dest_addr, (char*)addr, addr_len)) {
...
void theia_recvfrom_ahg(long rc, int fd, void __user *ubuf, size_t size, unsigned int flags, struct sockaddr __user *addr, int __user *addr_len)
{
  ...
  if (sock && (sock->type == SOCK_STREAM || sock->type == SOCK_SEQPACKET)) {
    ...
    if (addr != NULL && addr_len != NULL) {
      ...
      if (copy_from_user((char*)&pahgv_recvfrom->src_addr, (char*)addr, *addr_len)) {
      ...
      *((loff_t *)(pretval + sizeof(u_int))) = pos;
      ...
      do
      {
        ...
        if(args)
          ...
          TPRINT("record_read: can't allocate buffer for cpy_size\n");
          kfree(entry);
          return -ENOMEM;
        }
